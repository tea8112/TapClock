/*
    ...
    v0.2.2
    Bugfix: PM not shown between 12 and 1
    Change: Displaytime now function with parameter
    Change: System5x7.h font modified, chars 123-127 showing a small battery, 91/93 = PM/AM
    Change: Stats Page & Battery Values
...
{
  // main loop variables
  int act_hr, act_min, act_sec, act_year, act_month, act_day, act_hr12, act_dayofweek;
  ...
  switch (State) {
    ...
    case 61:
      ...
      if ((sec_changed && every10secs && usbConnected) || init_view) {
        ...
        oled.setCursor(0,0);
        if (percent ==  0) oled.print(char(123));
